...
function createStudyCard(s, { extraClass = '', linkText = 'Ver estudio' } = {}) {
  ...
  card.innerHTML = `
    ...
    <div class="absolute bottom-0 left-0 p-6 w-full">
      ...
      <div class="flex items-center gap-3">
        <a
          href="${(window.STUDY_DETAIL_PATH||'pages/studies/detail.html')}?id=${encodeURIComponent(s.id)}"
  ...
  card.addEventListener('click', () => {
    window.location.href = `${window.STUDY_DETAIL_PATH||'pages/studies/detail.html'}?id=${encodeURIComponent(s.id)}`;
  });
